...
export default function LiveTempStrip() {
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="inline-flex w-fit items-center gap-2 rounded-full border border-border bg-surface/70 px-3 py-1.5 text-sm backdrop-blur-sm">
        ...
        {status === "loading" && <span className="text-muted-foreground">Checking Mumbai now…</span>}
        ...
        {status === "unavailable" && (
          <span className="text-muted-foreground">Live air temperature unavailable</span>
        )}
      </span>
      <span className="max-w-md text-xs leading-relaxed text-muted-foreground">
        ...
        <Link href="/methodology#limitations" className="text-brand-teal hover:underline">
